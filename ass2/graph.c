...
//New functions added to get the outdegree of an edge and the name of the vertex 
...
//get name of vertex
//takes the index of the vertex to find 
...
// used vertexID to get index
// number of edges connected to a vertex 
int nE(Graph g, int i)
{	
	assert (i < g->nV);
	int v, count;
	count = 0;
	for(v = 0; v < g->nV; v++){
		if(i == v)continue;	//ignore self links
		if(g->edges[i][v] != 0){
			count++;
		}
	}
