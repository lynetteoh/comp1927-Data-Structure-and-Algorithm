...
int main(int argc, char *argv[])
{
	...
	*/
    ...
    double **cost = malloc(unionSize * sizeof(double*));
    for(i = 0; i < unionSize; i++) {

        cost[i] = calloc(unionSize, sizeof(double));
            for( j = 0; j < unionSize; j++) {

                int position = getIndex(URLs[i], rankTable[j], columnSize[j]); //get index
                //printf("pos:%d\n",position );
                if(position != 0)
                    cost[i][j] += fabs(((double)position/columnSize[j]) - ((double)(j + 1)/unionSize));
            }
    }

    double **d = malloc(unionSize * sizeof(double*));
    for(i = 0; i < unionSize; i++) {
        dist[i] = malloc(unionSize * sizeof(double));
    }

    int *negCycle = malloc((unionSize + 1) * sizeof(int));
    while (findAugCycle(negCycle, cost, position, unionSize, dist)){

        // swap the urls in the direction of the cycle
        j = urls[negCycle[0]];
        //printf("J:%d\n",negCycle[0]);
        for(i = 1; negCycle[i] != -1; i++){
            urls[negCycle[i-1]] = urls[negCycle[i]];
        }
        urls[negCycle[i-1]] = j;
    }
    ...
    for(i = 0; i < unionSize; i++) {
        totalScaledFootrule += cost[i][urls[i]->position];
        //printf("%.6f\n",distance );
    }


   // URLs = copySet(unionS); 
    //ermutation(position, 0, unionSize, finalP, argc, URLs, &totalScaledFootrule, columnSize, rankTable); 
    printf("%.6f\n", totalScaledFootrule);
    for(i = 0; i < unionSize; i++){
        //printf("%d\n",finalP[i]);
        pos = finalP[i];
    ...
    while(fscanf(rankfp, "%s", buff) != EOF){
    	if(strstr(buff, "url") != NULL){
    		if(!isElem(seen, buff)) {
    			...
    			enterQueue(q, buff);     //insert into queue
    		}
    	}
    }
...
{
    double total = 0.0,a = 0.0, b = 0.0, scaledFR = 0.0 ;
    int i, j, index;
    ...
    for(i = 0; i < unionSize; i++){
        for(j = 0; j < argc-1 ; j++){
            ...
            b = (double)(position[i]+1)/(double)unionSize;
            scaledFR = fabs(a-b);
            total += scaledFR;
        }
    }
...
int findAugCycle(int *negCycle, double **cost, int *position, int unionSize, double **dist) {
    ...
    for(i = 0; i < unionSize; i++){
        for(j = 0; j < unionSize; j++){
            ...
            dist[i][j] = cost[i][position[j]] - cost[i][position[i]];
            //printf("dCost:%.6f\n",dCost[i][j] );
            //printf("%s\n",urls[i]->name ); //urls is the array keep all the unique url
            //printf("Cost :%.6f\n",cost[i][j] ); distance from one to another
        }
